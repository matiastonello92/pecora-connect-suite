-- Create new user roles and permissions system for PecoraNegra (Fixed)
...
CREATE TABLE public.role_audit_log (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID, -- Made nullable to handle system updates
...
-- Function to log role/permission changes (fixed to handle null auth.uid())
...
BEGIN
  INSERT INTO role_audit_log (
    ...
    auth.uid(), -- This can be null during migrations
    COALESCE(NEW.user_id, OLD.user_id, NEW.id, OLD.id), -- Handle different table structures
    TG_OP,
    to_jsonb(OLD),
    to_jsonb(NEW)
  );
...
-- Update existing records with default values BEFORE creating triggers
...
-- Triggers for audit logging (created after data updates)
