-- Scalable Location Management Test Data Population Script (Fixed)
-- This script generates 10,000 locations with hierarchical structure and 100,000 users

-- First, create user_locations table for many-to-many relationship (if not exists)
...
-- Generate Countries (10 countries) - Only if they don't exist
...
-- Generate Cities in smaller batches to avoid memory issues
DO $$
DECLARE
    region_rec RECORD;
    city_batch INTEGER := 50; -- Process 50 cities at a time
...
BEGIN
    ...
    LOOP
        current_city := 1;
        WHILE current_city <= 99 LOOP
            INSERT INTO public.locations (code, name, parent_location_id, hierarchy, depth, path, is_active)
            SELECT 
                'test_city_' || ((region_rec.region_num - 1) * 99 + (current_city + i - 1)),
                CASE 
                    WHEN region_rec.region_num = 1 THEN -- Lombardy cities
                        CASE (current_city + i - 1)
                            WHEN 1 THEN 'Milan'
                            WHEN 2 THEN 'Bergamo'
                            WHEN 3 THEN 'Brescia'
                            WHEN 4 THEN 'Monza'
                            WHEN 5 THEN 'Como'
                            ELSE 'City ' || (current_city + i - 1) || ' in Lombardy'
                        END
                    WHEN region_rec.region_num = 11 THEN -- Île-de-France cities  
                        CASE (current_city + i - 1)
                            WHEN 1 THEN 'Paris'
                            ...
                            ELSE 'City ' || (current_city + i - 1) || ' in Île-de-France'
                        END
                    ELSE 'City ' || (current_city + i - 1) || ' in Region ' || region_rec.region_num
                END,
                region_rec.id,
                region_rec.hierarchy || jsonb_build_object('city', 
                    CASE 
                        WHEN region_rec.region_num = 1 THEN
                            CASE (current_city + i - 1)
                                WHEN 1 THEN 'Milan'
                                WHEN 2 THEN 'Bergamo'
                                WHEN 3 THEN 'Brescia'
                                ...
                                ELSE 'City ' || (current_city + i - 1) || ' in Lombardy'
                            END
                        WHEN region_rec.region_num = 11 THEN
                            CASE (current_city + i - 1)
                                WHEN 1 THEN 'Paris'
                                WHEN 2 THEN 'Boulogne-Billancourt'
                                WHEN 3 THEN 'Saint-Denis'
                ...
                3,
                region_rec.path || ('test_city_' || ((region_rec.region_num - 1) * 99 + (current_city + i - 1))),
                true
            FROM generate_series(1, LEAST(city_batch, 99 - current_city + 1)) AS i
            WHERE NOT EXISTS (
                SELECT 1 FROM public.locations 
                WHERE code = 'test_city_' || ((region_rec.region_num - 1) * 99 + (current_city + i - 1))
            );
            
            current_city := current_city + city_batch;
        END LOOP;
    END LOOP;
END $$;

-- Create mock users (100,000 users) in smaller batches
DO $$
DECLARE
    batch_size INTEGER := 500; -- Smaller batch size for better performance
    ...
    restaurant_roles TEXT[] := ARRAY['waiter', 'runner', 'bartender', 'floor_manager', 'cook', 'kitchen_assistant', 'pizza_chef', 'dishwasher', 'stock_manager'];
BEGIN
    WHILE current_batch * batch_size < total_users LOOP
        -- Check if users already exist to avoid duplicates
        IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE email = 'test_user_' || (current_batch * batch_size + 1) || '@example.com') THEN
            ...
            SELECT 
                ...
                'active',
                ARRAY['test_city_' || ((current_batch * batch_size + i - 1) % 9900 + 1)],
        ...
        -- Progress indicator
        IF current_batch % 50 = 0 THEN
            RAISE NOTICE 'Created % users so far...', current_batch * batch_size;
        END IF;
    END LOOP;
END $$;
