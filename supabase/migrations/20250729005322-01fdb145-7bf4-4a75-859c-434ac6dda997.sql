-- Hierarchical Locations System Migration
...
-- Step 6: Create materialized view for fast hierarchical queries
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_hierarchy_gin 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_depth 
ON public.locations (depth);

-- GIN index for path array (fast path-based queries)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_path_gin 
ON public.locations USING GIN (path);

-- Composite index for active locations with parent
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_locations_active_parent 
...
CREATE TRIGGER refresh_hierarchy_view_on_change
  AFTER INSERT OR UPDATE OR DELETE ON public.locations
  FOR EACH STATEMENT
  EXECUTE FUNCTION public.refresh_hierarchy_view_trigger();

-- Step 10: Initialize existing locations with hierarchy data
-- Update existing locations to set proper hierarchy for current data
DO $$
DECLARE
  loc RECORD;
BEGIN
...
-- Step 11: Create RLS policies for hierarchical access
-- Policy to allow viewing locations based on user's location hierarchy access
CREATE POLICY "Users can view locations in their hierarchy" 
ON public.locations FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM public.location_hierarchy_view lhv
    WHERE lhv.code = ANY(get_current_user_locations())
    AND (
      -- User can see their assigned location
      lhv.code = public.locations.code
      OR
      -- User can see parent locations in their hierarchy
      public.locations.code = ANY(lhv.ancestor_codes)
      OR
      -- User can see child locations in their hierarchy
      public.locations.code = ANY(lhv.children_codes)
    )
  )
);
